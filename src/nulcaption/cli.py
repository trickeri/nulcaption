...
def _caption(args: argparse.Namespace) -> int:
    ...
    print(f"[1/3] transcribing {src.name} ...")
    try:
        words = transcribe(
            src,
            ...
            prefer_service=args.config.use_stt_service and not args.no_service,
            service_url=args.service_url or (args.config.stt_service_url or None),
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    cap.add_argument("--vad-threshold", type=float, default=cfg.vad_threshold,
                     help="VAD speech-probability cutoff (whisper default 0.5). "
                          ...
                          "clean quiet mic track. Local backend only.")
    cap.add_argument("--no-service", action="store_true", dest="no_service",
                     help="don't use the shared system-wide STT service; always "
                          "run nulcaption's own local whisper.cpp backend")
    cap.add_argument("--service-url", dest="service_url", default=None,
                     help="override the STT service endpoint (default: "
                          "$WHISPER_HTTP_URL or http://127.0.0.1:48450/inference)")
